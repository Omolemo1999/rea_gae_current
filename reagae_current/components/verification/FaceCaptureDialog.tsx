...
export default function FaceCaptureDialog({
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      if (!token) throw new Error("Secure verification is still preparing. Please wait a moment and try again.");
          ...
          <Box sx={{ position: "relative", minWidth: 0, overflow: "auto" }}>
            ...
            <DialogContent sx={{ p: { xs: 2.5, sm: 4, md: 5 } }}>
              ...
              <Box sx={{ mt: 2.5, aspectRatio: "4 / 3", bgcolor: "#0A1020", position: "relative", overflow: "hidden", border: "1px solid #172033" }}>
                ...
                {started && <Box sx={{ position: "absolute", inset: "10% 25%", border: "2px solid rgba(255,255,255,.88)", borderRadius: "42%", boxShadow: "0 0 0 999px rgba(0,0,0,.20)", pointerEvents: "none" }} />}
              </Box>
