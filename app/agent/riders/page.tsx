...
export default function RiderReview() {
  ...
  return <AuthGuard role="AGENT"><DashboardShell role="AGENT"><Fade in timeout={500}><Box sx={{ minWidth: 0 }}>
    ...
    <Stack gap={2} sx={{ mt: 3 }}>
      {loading ? [1,2,3].map(i => <Paper key={i} sx={{ p: 3, borderRadius: 4 }} elevation={0}><Skeleton width="40%" /><Skeleton width="70%" /><Skeleton height={80} /></Paper>) :
      items.map(r => <Paper key={r.id} elevation={0} sx={{ p: { xs: 2, md: 3 }, border: "1px solid", borderColor: "divider", borderRadius: 5, minWidth: 0 }}>
        ...
        <Paper elevation={0} sx={{ mt: 2, p: 2, borderRadius: 4, bgcolor: "rgba(49,92,214,.045)", border: "1px solid rgba(49,92,214,.1)" }}>
          ...
          <Stack gap={1} sx={{ mt: 1.5 }}>
            {(r.rideVerifications || []).slice(0, 5).map((v: any) => <Stack key={v.id} direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1} sx={{ p: 1.2, borderRadius: 2.5, bgcolor: "white" }}>
